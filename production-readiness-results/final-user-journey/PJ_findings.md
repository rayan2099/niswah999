...
> **Status update (2026-09-06, Final User Journey / Upstream Write-Reliability Reconciliation wave):** This document is the frozen original audit (2026-09-04), preserved as-is for historical reference. Current status: `PJ-001` — **VERIFIED_CLOSED** (favorable — live-verified via direct `auth` schema trigger inspection during the Wave 0 remediation session, 2026-09-04, and re-confirmed via a fresh, authenticated live `supabase functions list` check this wave; `public.users` is populated for every real account by an untracked-but-live trigger — the residual concern that this provisioning logic exists only in the live database, not in version control, is tracked separately under `DI-001`/`ROOT-007`, not `PJ-001` itself). `PJ-002` — **PARTIALLY_REMEDIATED/OPEN** (the silent-failure/no-warning defect is closed; cross-device sync, part of this finding's own original scope, remains a separate, unaddressed architectural question — a narrative aside in an earlier wave calling this "VERIFIED_CLOSED" was an overclaim that didn't account for the finding's full native scope, corrected this wave). `PJ-003` — **OPEN**, re-confirmed unchanged via direct code trace this wave. `PJ-004` — **VERIFIED_CLOSED** (deployed to production as function version 8, live-verified via forced-failure testing against an isolated stack and a subsequent authenticated production recheck, both 2026-09-05; re-confirmed this wave via a fresh, authenticated `supabase functions list` check showing the same version 8 still live, unchanged — an intervening wave's claim that this was "not yet deployed" was a genuine documentation error, corrected here). `PJ-005` — **OPEN**, re-confirmed unchanged via direct code trace this wave (`community_board_screen.dart`/`profile_screen.dart` still branch to fabricated demo conversations whenever `NiswahSupabase.clientOrNull?.auth.currentUser?.id` is null, which a dying session — not just deliberate pre-login browsing — produces; an earlier wave's claim that this was "independently fixed" was also a documentation error, corrected here). `PJ-006` — **VERIFIED_CLOSED** (Doctor's Report Data Completeness + Truthfulness wave, 2026-09-06). Check `production-readiness-results/master/00_04_MASTER_FINDING_REGISTER.md`, not this file, for current status.
