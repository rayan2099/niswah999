...
**Last updated:** 2026-09-08, RD-009 Emergency Release Verification wave. `RD-009` is now **`VERIFIED_CLOSED`** — a real signed emergency artifact was produced on live GitHub Actions infrastructure (run `34216345677`), independently re-verified (checksum, signature, package ID, versionCode, 90-day retention) rather than trusted from the workflow's own output. One genuine CI-portability bug was found and fixed along the way (`scripts/generate_release_manifest.sh` hardcoded macOS-only paths that killed it silently on the Linux CI runner). See `production-readiness-results/master/00_09_PHASE1_ROOT_CAUSE_REMEDIATION_PLAN.md` §47 for full evidence. Supersedes the Production Rollback / Rapid Recovery Capability wave's version (2026-09-06), which drilled the mechanism locally and got the workflow running through its intended secrets gate on real infrastructure, but had not yet produced a real signed artifact.
...
- **`RD-006`, ordinary/routine-release build-number process**: still open, separately from `RD-009`. A documented or automated process for incrementing the build number on *every* store submission (not just the emergency path, which is now proven) does not yet exist, since no ordinary/routine release pipeline exists yet either. Deferred until that pipeline is built.
