...
**Last updated:** 2026-09-08, RD-006 Routine Release wave. `RD-006` is now also **`VERIFIED_CLOSED`** — a new, manually-triggered `.github/workflows/routine-release.yml` computes a collision-free, monotonically increasing build number (`100 + github.run_number`) for ordinary/routine releases (as opposed to `RD-009`'s emergency path), proven with two real sequential runs on live GitHub Actions (`build_number=101` then `102`, both independently re-verified on both Android and iOS artifacts). See §12 below and `00_09` §48 for full evidence. `RD-009` was closed the same day (previous wave) — a real signed emergency artifact was produced on live GitHub Actions infrastructure (run `34216345677`), independently re-verified (checksum, signature, package ID, versionCode, 90-day retention). One genuine CI-portability bug was found and fixed along the way (`scripts/generate_release_manifest.sh` hardcoded macOS-only paths that killed it silently on the Linux CI runner). See `00_09` §47 for that evidence.
...
## 9. Emergency workflow (Phase K) — VERIFIED_CLOSED, RD-009 Emergency Release Verification wave, 2026-09-08
...
**Status: `VERIFIED_CLOSED`.** The owner configured all three previously-missing secrets (names verified via the GitHub API, values never seen). Triggered again via a real `workflow_dispatch` API call: **first run (`34214731608`) failed** — but productively, at a new, previously-unexercised step: checkout, secret-reconstruction (`.env`, keystore, `key.properties`), analyze/test, the actual signed Gradle build, and signing-certificate verification (real `CN=Niswah` cert) all passed for the first time on real infrastructure; failure was in `scripts/generate_release_manifest.sh`'s checksum/manifest step — a genuine CI-portability bug (hardcoded macOS-only paths for locating `apksigner`, killed the script silently under `set -euo pipefail` on the Linux runner). **Fixed and verified locally**, then pushed. **Second run (`34216345677`, `build_number=5`) completed `success` end-to-end** — all 14 real steps, including artifact upload. Independently re-verified (not merely trusted from the run's own log): downloaded the artifact and re-computed its SHA-256 (matched GitHub's own digest exactly), independently re-ran `apksigner verify` (same real `CN=Niswah` certificate, SHA-256 `6d888f0166f7897098fbdd3229ca7c441ec80428741ac0592e5504ab51726ecd`, identical across both runs — never weakened or substituted), independently ran `aapt dump badging` (`com.niswah.niswah`, `versionCode=5`, strictly greater than every prior known value), and confirmed 90-day retention via the artifact's own API metadata (`expired: false`). No Play Store/App Store publication occurred. Full evidence: `00_09` §47.
...
`CFBundleVersion` is driven by the same `pubspec.yaml` `+N` field (or a `--build-number` override) as Android's `versionCode` — confirmed via direct inspection of the compiled `Info.plist`. `scripts/generate_release_manifest.sh` now supports iOS artifacts (`.app` directories and `.ipa` files) — tested this wave against the real `--no-codesign` build output: correctly hashes the `.app` bundle's full contents, correctly reports `"UNSIGNED (--no-codesign build)"` via `codesign -dvvv` rather than fabricating a certificate identity. An iOS emergency rebuild would follow the exact same `git worktree` + higher-build-number pattern already drilled for Android (§4.2) — not re-drilled separately this wave since the underlying mechanism (Flutter's build-number handling) is identical and already proven; the only iOS-specific addition is that `flutter build ipa` (not `apk`/`appbundle`) would be the actual command, requiring real signing to produce a distributable artifact. **This unification was directly exploited by the routine release workflow (§12) — `routine-release.yml`'s single computed build number is fed to both platforms simultaneously and confirmed identical in both artifacts (`versionCode=102` / `CFBundleVersion=102`), with no platform-specific reconciliation logic needed.**

---

## 12. Routine release workflow (`RD-006`, RD-006 Routine Release wave, 2026-09-08)

`.github/workflows/routine-release.yml` — `workflow_dispatch`-triggered only (manual, explicit control; never runs on push/PR), 3 jobs: `analyze-and-test` (checkout, `dart analyze`/`flutter test` gate, computes the build number), `build-android` (signed release APK), `build-ios` (no-codesign release `.app`, `DC-010`'s real signing deliberately out of scope). Distinct from `emergency-release.yml` (§9): that workflow is for an urgent, arbitrary-commit rebuild during an incident with an operator-chosen build number; this one is the ordinary, repeatable release path with a build number CI computes on its own.

**Build-number strategy**: `100 + github.run_number` — a real, GitHub-native, monotonically increasing, never-reset, never-reused per-workflow counter. No committed counter file, no external state, no local machine dependency. The `100` offset is a permanent, deliberate reserved-range convention: values 1-99 stay the emergency workflow's small, operator-chosen numbers; this workflow structurally cannot produce a value in that range, so the two mechanisms cannot collide by construction.

**Secrets**: reuses the exact same three secrets `emergency-release.yml` already uses (`EMERGENCY_BUILD_ENV_FILE`, `ANDROID_RELEASE_KEYSTORE_BASE64`, `ANDROID_KEY_PROPERTIES`) — no new secret was created. There is only one real production `.env` and one real release keystore; both workflows correctly sign with that same identity.

**Status: `VERIFIED_CLOSED`.** Two real sequential `workflow_dispatch` runs on live GitHub Actions, not simulated:

| Run | Run ID | `run_number` | Computed `build_number` | Result |
|---|---|---|---|---|
| 1 | `34218712700` | 1 | 101 | `success` — all steps, all 3 jobs |
| 2 | `34220261565` | 2 | 102 | `success` — all steps, all 3 jobs |

`102 > 101 > 5` (the highest value from any prior mechanism across this entire engagement — pubspec `2`, a local drill `3`, a failed emergency attempt `4`, the verified emergency release `5`). Both runs' artifacts independently re-downloaded and re-verified, not trusted from the workflow's own log: Android — real `CN=Niswah` signing certificate (SHA-256 `6d888f0166f7897098fbdd3229ca7c441ec80428741ac0592e5504ab51726ecd`, identical to every prior confirmed instance across this engagement, the same real production key), `com.niswah.niswah` package ID, `versionCode=102`, `versionName=1.0.0` (semantic version correctly untouched); iOS — `CFBundleVersion=102` (identical to Android's value), `CFBundleShortVersionString=1.0.0`, `CFBundleIdentifier=com.niswah.niswah`, honestly confirmed unsigned via `codesign -dvvv` (`DC-010` genuinely not touched). Both artifacts confirmed retained 90 days via API metadata, `expired: false`. No Play Store/App Store publication occurred in either run — no such step exists in the workflow, by design.

**`RD-006` = `VERIFIED_CLOSED`.** `DC-010` remains independently open — RD-006's own native bar never required real Apple signing (confirmed directly from `RD_remediation_plan.md` R1.3's retest text: "simulate two successive builds," no mention of store submission or signing), and this workflow did not attempt, simulate, or fabricate it.
...
- ~~Configure `ANDROID_RELEASE_KEYSTORE_BASE64` / `ANDROID_KEY_PROPERTIES` / `EMERGENCY_BUILD_ENV_FILE` as GitHub Actions secrets.~~ **Done (RD-009 Emergency Release Verification wave, 2026-09-08)** — configured by the owner, verified by name only, and used successfully to produce a real signed, checksummed, retained emergency artifact. `RD-009` is now `VERIFIED_CLOSED`.
- ~~`RD-006`, ordinary/routine-release build-number process.~~ **Done (RD-006 Routine Release wave, 2026-09-08)** — `.github/workflows/routine-release.yml` now computes a real, collision-free, monotonically increasing build number for every routine release automatically; proven with two real sequential runs (§12). `RD-006` is now `VERIFIED_CLOSED`.
- **iOS signing (`DC-010`)**: the single remaining gap after this wave's full precheck — sign into Xcode with an Apple ID enrolled in the Apple Developer Program and select the Team for `Runner` (§11.1). Not fabricable; everything else autonomously verified working. **Explicitly not required by `RD-006`'s own native closure bar** (confirmed by direct reading — its own retest is satisfiable via an unsigned build) — remains open independently, unaffected by `RD-006`'s closure.
