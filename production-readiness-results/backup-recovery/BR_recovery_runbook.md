...
## 7. RPO / RTO (see §20 Phase G, §34, §35 in `00_09` for full derivation)
...
- **RTO (real production data):** **NOT YET TESTED** — no real backup exists yet to restore-test against (see §8); once one exists, a genuine restore-to-new-project drill is required before any RTO figure can be reported honestly.
...
- **RPO (production user data):** **infinite / total loss today.** A minimum-safe mechanism has been selected (Supabase Pro-tier automatic daily managed backups — see §35) but **not yet provisioned**, since it requires a billing/plan change only the project owner can make. Once provisioned, platform-theoretical RPO becomes up to 24 hours (one backup per day). PITR was deliberately not selected as the default — no RPO requirement in this engagement justifies its added cost over daily backups. See `00_09` §35 for the full owner-action-minimization reasoning.

## 8. What this runbook cannot do — stated plainly

- **It cannot recover real user data, today.** The canonical baseline contains schema only — table structure, constraints, functions, triggers, RLS policies. It was deliberately built without exporting any production row data, per the operating constraints of every wave in this engagement. If the live database were lost today, everything reproducible by this runbook would come back; every real user's cycle logs, pregnancy data, chat history, and account would not. **The single owner action that changes this** (confirm/upgrade to Supabase Pro and allow the first daily backup to generate — `00_09` §35 Phase D) has been identified but not executed as of this writing.
