...
- **It has not yet had a real restore proven against it.** A real production backup now exists (7 daily `COMPLETED` physical backups, confirmed 2026-09-08) — a genuine, hard-won change from every prior wave's "zero backups" state — but no restore drill has ever been run against it. Until one is, "a backup exists" and "we can actually recover from it" remain two different claims, and this runbook does not conflate them. **The remaining owner action — precisely determined 2026-09-08 by direct inspection of the Supabase Management API's own OpenAPI spec, not assumed**: no Management-API-drivable "restore this backup into a new project" endpoint exists — every backup/PITR restore endpoint (`/database/backups/restore`, `/database/backups/restore-pitr`) operates in-place on the source project only, and `/projects/{ref}/restore` is unrelated (project un-pause, not backup restore). Two real paths exist, both outside this engagement's current authorization: **(A)** the org's Pro plan is entitled to "restore to new project" (`backup.restore_to_new_project` entitlement confirmed `true`), but it is a **Supabase Dashboard/Studio-only** action — a human must click through the dashboard UI; no API token can drive it. **(B)** Database Branching with `with_data: true` can create an isolated, production-data-cloned environment via the API, but requires provisioning a billed compute-instance addon (from ~$0.01344/hour) — a real, owner-authorized billing action, consistent with every other billing-adjacent decision in this engagement. **Exact next step**: the owner either (a) performs the restore-to-new-project drill directly in the Supabase Dashboard (Database → Backups → select a backup → Restore to new project), or (b) explicitly authorizes creation of a billed branch compute instance so this can proceed via the API — then in either case, re-run the existing 14-point behavioral verification suite (§5-adjacent, `00_09` §20 Phase E) against the restored/branched environment, then destroy it. This is the one step left to fully close `BR-001` per its own native definition.
