...
**Last updated:** 2026-09-08, BR-001 Restore Drill Wave. **Last schema restore test:** 2026-09-06 — two full rebuilds (baseline-only, baseline + `W1-001`), both schema-contract-verified — see `production-readiness-results/master/00_09_PHASE1_ROOT_CAUSE_REMEDIATION_PLAN.md` §37. **Last real production data restore test: 2026-09-08** — the owner restored a real `COMPLETED` physical backup (`2026-09-07T16:24:18.189Z`) into an isolated project (`niswah-br001-restore-drill`, ref `rpopudibfpoefejyarhe`); the canonical 14-point recovery suite was independently executed against it with real restored production data (23 users, 43 `cycle_entries`, etc.) — 12/14 full PASS, 1 correctly-expected partial (`W0-003` reproduced, confirming fidelity), 1 schema-level PASS. **`BR-001` = `VERIFIED_CLOSED`** — see `00_09` §46 for full evidence. The restored project itself has since been deleted by the owner (confirmed 2026-09-08 via a read-only Management API check — `rpopudibfpoefejyarhe` no longer exists, "Resource has been removed"); no cleanup action remains outstanding for this finding.

This runbook covers **schema/application recoverability** — rebuilding the database structure and confirming the application works against it — and now also tracks **real production backup existence and a proven real-data restore**, closing the gap this document previously described as outstanding. See §7 (RPO/RTO) for the precise, current, evidence-backed state of each.
...
- **RTO (real production data):** **measured, 2026-09-08 — ≈13 minutes**, from the restored project's platform-confirmed availability to full 14-point behavioral verification passing. Honestly caveated: this is measured from the restored project's `created_at` timestamp (this session's first confirmable evidence it existed), not from the owner's actual restore-initiation click, which was not visible to this session and was not invented as a number. See `00_09` §46 for the full evidence trail.
...
- **A real restore has now been proven against it (2026-09-08).** The owner restored a real `COMPLETED` physical production backup (`2026-09-07T16:24:18.189Z`) via the Supabase Dashboard's "restore to new project" action into `niswah-br001-restore-drill` (ref `rpopudibfpoefejyarhe`). This session independently verified it — 24/24 tables, 8/8 functions, 24/24 RLS-enabled, both `auth.users` triggers present, real production data volumes intact (23 users, 43 `cycle_entries`, etc.), and the canonical 14-point recovery suite executed against real restored data: 12/14 full PASS, 1 correctly-expected partial (`W0-003` reproduced, confirming fidelity, not a new defect), 1 schema-level PASS. **`BR-001` = `VERIFIED_CLOSED`** — "a backup exists" and "we can actually recover from it" are no longer two different, unproven claims; the second has now been demonstrated with real evidence. **No owner action remains for this finding**: the temporary restored project (`niswah-br001-restore-drill`, ref `rpopudibfpoefejyarhe`) — deliberately not deleted by this session at the time (an irreversible action against a resource this session did not create) — has since been deleted by the owner directly, confirmed 2026-09-08 via a read-only Management API check. See `00_09` §46 for the full evidence trail.
