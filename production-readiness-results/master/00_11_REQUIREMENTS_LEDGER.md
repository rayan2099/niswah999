...
- **Severity**: self-scored per-item in the traceability matrix (`VERIFIED`/`PARTIAL`/`MISSING`/`N/A`) — not a single blanket severity, since sub-items range from fully verified (Hardening 5's mutation boundary, live-Postgres-proven) to genuinely not built (Commit F7/F8's visual semantics and calendar UI, Commit H's remaining ~12-15 adversarial-matrix items).
...
- **Evidence level**: mixed E2 (unit/widget tests, `flutter analyze`/`format` clean, 616 passing/10 pre-existing-failure baseline unchanged) and E3 (every SQL migration/RPC verified against a real local Postgres reconstruction, inside real transactions, with real `authenticated`-role/JWT simulation, never superuser). **Zero E4** (real owner/device) evidence exists for any part of this charter — explicitly not claimed anywhere in the contract doc.
