...
- **Update, 2026-09-20 (Notification Cancellation Closure wave, same day, later)**: three real gaps closed in when the 30-day window actually gets *cancelled* — disabling the preference and a canonical refresh verifying a remote episode end both previously left an already-scheduled window outstanding; a new bounded sweep (re-deriving candidates from existing episode data, no new persisted state) now cancels genuinely, gated on the platform acknowledging each cancel call; the Settings toggle reconciles the scheduler immediately rather than waiting for app resume; a stale-day sweep and centralizing `refreshLocalTimezone(forceRefresh: true)` inside `refresh()` itself close the reminder-time/timezone-change gap for every caller, not only `main.dart`'s resume handler. Closure Blocker 1 discipline re-verified: a genuine read failure never triggers cancellation. Eight new integration tests, against the real coordinator with a mocked plugin channel, cover exactly the required scenarios. Final count: 747 tests, 10 failures, zero PR-introduced regressions (same 10 golden/parity failures as the prior wave's merge-base-verified baseline). BR-002 migration reproducibility reconfirmed passing. Full narrative: `docs/menstrual-data-integrity-contract.md` §12.
- **Update, 2026-09-21 (Local Notification Reconciliation Closure wave)**: closed the remaining real gap — the prior wave's own cancellation sweeps still called `getEpisodesForUser` (a Supabase read) to decide what to cancel, so an explicit local opt-out could silently fail to take effect while offline or mid-outage. Replaced with reconciliation against the OS's own actual pending-request set: a new `NotificationService.pendingActiveBleedingReminders()` wrapper decodes each request's existing opaque payload purely locally (never touching Supabase, never exposing plugin internals); the disable and verified-no-open-episode paths now make zero Supabase calls; the eligible-episode path reconciles `actualOwnedPendingIds - desiredIds` in one pass, subsuming the prior wave's separate today's-id and stale-day sweeps and correctly handling a shifted window from any cause. Twelve integration tests (replacing the prior 8) prove every required scenario, including cross-user isolation and malformed-payload safety. Final count: 751 tests, 10 failures, zero PR-introduced regressions. BR-002 migration reproducibility could not be re-run to a clean pass this wave — three genuine attempts each hit an identical docker container health-check timeout in this sandbox before the script's own validation logic ran; zero SQL touched this wave, so the schema contract itself is unaffected. Full narrative: `docs/menstrual-data-integrity-contract.md` §13.
