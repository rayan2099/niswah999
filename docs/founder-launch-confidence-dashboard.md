...
### 🔴 REOPENED — you tested it and it failed, root cause now known (2026-09-15)

- **Madhhab "I don't know" option** (`AUTH-010`) — you tested it on a real phone and it still showed only the original 4 choices. The fix is genuinely in the code (re-checked and re-tested this pass) but your phone isn't running that code — see the update at the top of this document for the full explanation. Moved back from 🟡 to 🔴 because your test is what counts, not the earlier automated pass. Needs the delivery issue resolved, then a real retest, before this can move forward again.
- **Madhhab only saved on your phone, silently defaults to Hanbali on reinstall** (`AUTH-005`) — the database half of this fix is live regardless; the app half has the same not-yet-delivered problem as `AUTH-010` above, since it's the same code. Also needs the delivery issue resolved before a real retest means anything.
...
**Reopened, 2026-09-15 — your real-phone test failed, root cause is a delivery gap, not the fix itself**:
4. **`AUTH-005`, the Madhhab silent-default/persistence bug.** The database half is genuinely live and unaffected. The app half is built and re-confirmed correct in the code today, but is sitting in the same not-yet-delivered working copy as `AUTH-010` below — your phone isn't running it. Reopened, not because the fix is wrong, but because your test is the standard that decides this, and it failed.
5. **`AUTH-010`, the "I don't know my madhhab" gap.** Built, re-checked, and re-tested automatically today — and still failed your real-phone test, because your phone isn't running this code yet. See the update at the top of this document for the full explanation. Reopened.
...
**Two things were done this pass: a real, working fix for the "Use Current Location" confirmation problem (`AUTH-011`), and a full investigation of why your Madhhab retest failed.** The investigation's answer isn't a code bug — it's that this entire project's work has been living on a working copy that was never delivered to wherever your actual app builds come from, so your phone has effectively never run any of this project's fixes, Madhhab included. **The one thing that needs your input now is how you'd like that work delivered** — that's a decision about your project's setup, not something to make unilaterally on your behalf. Once it's resolved, `AUTH-005` and `AUTH-010` are ready for an immediate real retest (the code itself needs no further changes), and `AUTH-011` is ready for its first one.
