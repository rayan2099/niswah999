...
The period-tracking rebuild described in the launch confidence dashboard (`docs/founder-launch-confidence-dashboard.md`'s 2026-09-19/2026-09-20 updates) lives entirely on PR #4 (`feat/menstrual-data-integrity`), which is deliberately kept in **draft** and has **not been merged**. There is still nothing for you to do right now — no owner action, no CI/production change, and this PR is not being merged as part of this update. **One thing has changed, 2026-09-19 (later the same day)**: a real, portable retest script now exists in the repository at `docs/owner-e4-menstrual-journey.md` (a plain markdown file — not a link that can go stale or expire), covering the full live-device journey (onboarding through account switch) with plain pass/fail criteria and no SQL/log/database steps asked of you. **Updated again, 2026-09-20**: an independent review found its multi-day notification-delivery step was chained together with a same-run notification tap, which would have reopened the app partway through and quietly invalidated the "uninterrupted for 5 days" test it was meant to be — split into two separate steps (delivery only, then tap-through separately) so each measures only what it claims to. **Updated again, 2026-09-20 (later the same day)**: a real gap in reminder *cancellation* was found and closed — turning the reminder off, or a period genuinely ending, previously didn't reliably cancel reminders already sitting on the phone; this is now fixed and tested (see the dashboard's own latest update for the plain-language version). No new owner action or retest step is needed for this specific fix — it isn't something a live-device pass/fail step could distinguish from "the reminder just never fired," since both look identical to the person testing; closure here rests on the automated coordinator tests, disclosed as such. It is committed now so it's ready and reviewable whenever you decide to look at this PR — using it before then isn't necessary, since the app you actually have is unaffected either way. When you're ready to review the PR for merge, `docs/menstrual-data-integrity-contract.md` is the full technical reference, and this section will be replaced with an active retest handoff at that point, following the same pattern as every other handoff in this document.
...
## Startup Recovery Handoff (new, 2026-09-15, Startup Auth-Gate Infinite-Spinner Investigation) — `AUTH-012`, `E2` DONE, `E4` NEEDS YOU

**What you reported**: on a clean rebuild from the now-current `main`, the app opened to a white screen, then a loading spinner (rendering correctly — this is not the earlier `UI-001` spinner-rendering bug) that never stopped. No Sign In, no Onboarding, no Dashboard.

**What this session found**: a real flaw in how the app decides "is this person already signed in, and have they finished onboarding?" That check normally finishes in under a second. But if it ever failed — a network hiccup, or (the most likely cause here) a sign-in left over on your device from an old test account that no longer exists — the app had no way to tell "still checking" apart from "the check just failed." Both looked identical internally, so the app just kept showing the loading screen forever, with no error, no timeout, and no way for you to do anything about it.

**What was fixed**: the app now recognizes a failed check as its own real, distinct situation, separate from "still checking." When it happens, you'll now see an actual message — "We couldn't load your account. Check your connection and try again." — with two real buttons: **Retry** (tries the check again) and **Sign out** (starts fresh, which is the right move if the problem is a leftover sign-in from an account that no longer exists). You will never again see just a spinner with no way forward.

**Why your retest needs the same, currently-stuck device**: this bug depends on whatever specific condition is making the check fail on your phone/simulator right now — most likely a leftover sign-in. If you wipe the device and reinstall before retesting, you'd very likely get a fresh, working sign-in and never actually exercise the fix at all — you'd only prove the bug doesn't happen on a brand-new install, which isn't the same thing as proving it's fixed.

**What this session could not do**: reproduce your exact stuck session directly — there was no access to your simulator/device from this environment. The fix was proven with 12 automated tests that simulate every version of the failure (a failed check, a slow check, a check that times out, retrying after a failure, signing out mid-check, switching accounts mid-check) and confirm the app always recovers or shows a real error — never an endless spinner. But the very specific condition on your device (most likely a stale sign-in) could not be directly inspected or reproduced here.

**Your retest, once you have a moment** (about 2 minutes, using the SAME stuck phone/simulator — do not wipe it first):

1. **Confirm the current stuck state, if you haven't already**: note that the app is currently showing the endless spinner, on which device/simulator, and roughly how long you let it spin before giving up (for the record).
2. **Update to the latest code** (rebuild from the current `main`, same as before) and relaunch the app on that same device — do not delete/reinstall it, do not clear its data.
3. **Observe what happens now**: either (a) the app now correctly reaches Sign In, Onboarding, or your Dashboard (meaning the underlying check actually succeeds this time), or (b) you see the new "We couldn't load your account" message with Retry and Sign out buttons (meaning the check is still failing, but now you have a real way forward instead of a dead end).
4. **If you see the new message**: tap **Retry** once — does it succeed on the second try? If it still fails, tap **Sign out** — does that take you to a real Sign In screen (not another spinner)? Then sign back in — do you reach your Dashboard normally?

Report PASS/FAIL for each step. **PASS looks like**: you're never stuck on an endless spinner — you either reach a real screen directly, or you reach the new error message with working buttons that eventually get you to a real screen. If all steps pass, `AUTH-012` closes as `VERIFIED_CLOSED / E4`.

---

## Madhhab Authority Handoff (new, 2026-09-14, Fiqh Remediation Wave 1) — `AUTH-005` / `AUTH-010`, `E3`/`E2` DONE, `E4` NEEDS YOU

**🟡 Update, 2026-09-15 — you ran the retest below and it FAILED: your phone showed only the original 4 choices, no "I don't know" option.** This was a real result. The investigation that followed found the cause is not a defect in the fix itself: the code was re-read line by line and its full automated test suite re-run, both confirming the fix is genuinely present and correct in this project's working copy. The actual problem is bigger — **this project's entire body of work has been sitting on a working copy of the code that has never been sent to wherever your real app builds come from.** GitHub's own "main" copy of the project had been frozen since before this project began. So the app on your phone was never running any of this project's fixes — the Madhhab screen just happened to be the first place that became visible to you. **Both findings' status is corrected accordingly** (see below and the finding register): your screenshot is kept on record as accurate evidence of an old build, not as evidence the fix is broken, and both findings sit at their pre-test `E3`/`E2` (automated-verified) status again, awaiting a new retest.

**🟢 Update, 2026-09-15 (later the same day) — merged.** You reviewed and merged the pull request bringing `main` fully current with `terminal`'s work (reviewed beforehand for merge conflicts — none, secrets — none found, and destructive migrations — none; the two new migrations are additive and were already live on the production database). The retest script below is now meaningful for the first time — go ahead whenever you're ready.
