...
## 🟡 Read this first, 2026-09-15 (updated later the same day) — your real-device Madhhab retest failed, the root cause changes what "complete" above means, and a fix for the delivery gap is now in progress

**Update, same day**: the delivery gap described below is being closed. `main` (GitHub's copy) was found to have already advanced somewhat on its own, and a pull request is now open bringing it fully current with everything in this document. Nothing has merged yet — that's still your call, on GitHub, whenever you're ready. Once it merges, every "needs your retest" item below becomes meaningful for the first time.
...
**🟡 Update, 2026-09-15 (later the same day) — a pull request is now open to close the delivery gap.** `main` had already advanced partway on its own since the update above was written (the gap was smaller than first measured, though still real). A pull request bringing `main` fully current with `terminal` — reviewed first for merge conflicts (none), secrets (none found), and destructive migrations (none; the two new migrations are additive and already live on the production database) — is now open on GitHub. **Nothing below should be retested yet** — it still needs your review and merge on GitHub first; a retest against the still-undelivered code would only reproduce the same FAIL for the same reason. Once you merge it, both findings are ready for an immediate real retest with no further code changes expected.
