...
**Detection added (not auto-repair)**: `scripts/check_ai_rate_limit_objects.sh` — a non-mutating, read-only health check you can run manually or wire into your own monitoring/cron to catch this specific failure mode before real users do. It was deliberately not wired into this repo's CI, since that would require provisioning a new credential outside this wave's authorized scope.
