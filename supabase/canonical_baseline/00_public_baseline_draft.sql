-- =========================================================================
-- NISWAH CANONICAL DATABASE BASELINE (DRAFT — PROPOSED, NOT DEPLOYED)
-- Generated: 2026-09-04, from a live, read-only schema-only capture of the
-- production Supabase project (jkmjobvxfrmuwafczvtw), per Wave 0 of
-- production-readiness-results/master/00_09_PHASE1_ROOT_CAUSE_REMEDIATION_PLAN.md.
--
-- THIS FILE HAS NOT BEEN APPLIED TO PRODUCTION. It exists to prove, via an
-- isolated clean-rebuild test, that application-owned schema and behavior
-- can be reproduced from version control. Migration-ledger registration
-- against the live project requires a separate, explicit approval — see
-- the Wave 0 execution report for the exact proposed command.
--
-- DETERMINISM NOTE (plan §4.0.2): statements below are plain CREATE TABLE /
-- CREATE FUNCTION / CREATE TRIGGER — NOT "IF NOT EXISTS" / "OR REPLACE" —
-- so that applying this against any environment with unexpected existing
-- state fails loudly rather than silently succeeding. The ONE exception is
-- `CREATE SCHEMA IF NOT EXISTS "public"` a few lines down, left unmodified
-- because the `public` schema always exists in any real Postgres/Supabase
-- instance; a bare `CREATE SCHEMA "public"` would only ever fail there,
-- carrying no useful drift signal.
--
-- SCOPE: this file reproduces application-owned objects only (tables,
-- columns, constraints, indexes, RLS policies, application functions/RPCs,
-- and application-relevant triggers). Supabase-platform-managed schemas
-- (auth's own internal tables, storage's internal machinery, the
-- extensions themselves) are NOT reproduced here — see the Wave 0
-- execution report's Output A/B for what those are and why they're
-- excluded, per plan §4.0.1.
--
-- OBJECT CLASSIFICATION (full detail + evidence in the Wave 0 execution
-- report, Output B). Summary:
--   REQUIRED_APPLICATION_OBJECT: chat_messages, chat_threads,
--     community_comments, community_likes, community_posts, cycle_entries,
--     dream_entries, flagged_conversations, pregnancy_profile,
--     private_conversations, private_messages, profiles, users,
--     wellbeing_logs, and all 8 functions below.
--   REQUIRED_APPLICATION_OBJECT, NAME MISMATCH (see W0-001/W0-002 in the
--     execution report — the app currently queries a DIFFERENT, nonexistent
--     table name for each): prayer_log (app code queries 'prayer_entries'),
--     pregnancy_records (app code queries 'pregnancy_milestones'). Both
--     preserved here under their LIVE name, unchanged — renaming either the
--     table or the app code is a deliberate, later, reviewed decision, NOT
--     bundled into this capture-reality-as-is baseline.
--   UNREFERENCED / CANDIDATE_FOR_LATER_REMOVAL (known purpose per the
--     original audit's DI-008, confirmed still unreferenced by any current
--     code path): adah_ledger, cycle_logs, istihadah_episodes,
--     nifas_records, ramadan_records, symptoms_log.
--   UNKNOWN — REVIEW REQUIRED (purpose NOT documented in any tracked
--     source, NOT referenced by any current code path, exists live only):
--     chat_history, secret_vault. Preserved here (not destructive), flagged
--     for release-owner review before any future change touches them.
-- =========================================================================
...
CREATE FUNCTION "public"."can_access_user"("row_user_id" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE
    AS $$
...
CREATE FUNCTION "public"."is_conversation_participant"("conversation" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  SELECT EXISTS (
...
CREATE FUNCTION "public"."touch_private_conversation"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
CREATE TABLE "public"."adah_ledger" (
...
CREATE TABLE "public"."istihadah_episodes" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "start_date" "date",
    "end_date" "date",
...
CREATE TABLE "public"."nifas_records" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
...
CREATE TABLE "public"."ramadan_records" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
...
CREATE TRIGGER "trg_touch_private_conversation" AFTER INSERT ON "public"."private_messages" FOR EACH ROW EXECUTE FUNCTION "public"."touch_private_conversation"();
...
-- =========================================================================
-- AUTH-TO-PUBLIC INTEGRATION (live-only prior to this baseline — see
-- Wave 0 execution report, Output G / UNK-006 discussion in 00_05).
-- These two triggers on Supabase's own auth.users table are what makes
-- signup provisioning work today; neither appears in any tracked migration
-- or schema.sql before this baseline. Preserved verbatim, not consolidated.
-- =========================================================================

CREATE TRIGGER "auth_users_create_profile" AFTER INSERT ON "auth"."users" FOR EACH ROW EXECUTE FUNCTION "public"."create_user_profile"();
